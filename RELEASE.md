...
Commits the version bump, changelog, and bandit report (`Release vx.y.z`), then cleans `dist/` and builds wheel + sdist.
...
Commits distributables (`Distributables for vx.y.z`), uploads to PyPI, creates a git tag `vx.y.z`, and pushes both to origin.
...
| `scripts/build.sh`        | Commit release files, clean dist/ and build          | Fully                |
| `scripts/publish_test.sh` | Upload to Test PyPI, optional install verify         | Fully                |
| `scripts/publish.sh`      | Commit distributables, upload to PyPI, tag, git push | Fully (with confirm) |
