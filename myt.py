...
def is_date_short_format(string):
    """
    To determine if the string is expected shortformat. The short format
    is used by the program to make relative adjustments to a date.
...
def convert_to_date(refdate, num, timeunit="days"):
    """
    Returns a date post applying a relative adjustment to a reference date
...
def convert_hide(value, due):
    ...
    if value and is_date_short_format(value):
        ...
        elif not due is None and value[0:1] == '-':
...
@click.option('--desc', '-de', type=str,
              help="Short description of task", )
@click.option('--due', '-du', type=str,
              help="Due date for the task", )
@click.option('--hide', '-hi', type=str, 
              help="Date until when task should be hidden from Task views",)
@click.option('--group', '-gr', type=str,
...
def start(filters):
    ...
    connect_to_tasksdb()
    if potential_filters.get("all") == "yes":
        if not yes_no("No filters given for starting tasks, are you sure? (yes/no)"):
            exit_app(0)
...
def done(filters):
    ...
    connect_to_tasksdb() 
    if potential_filters.get("all") == "yes":
        if not yes_no("No filters given for marking tasks as done, are you sure? (yes/no)"):
            exit_app(0)
...
def mark_task_inprogress(potential_filters):
    ...
    """
    task_uuid_and_version = [element for itm in uuid_version_results for element in itm]
...
def get_tasks(task_uuid_and_version):
    """
    Gets the task details for a list of task uuid and versions.
        0 - id
        1 - version
        2 - description
        3 - status
    ...
    sql_tasks = """
        ...
        case when ws.due<date('now') then 'OVERDUE' when ws.due=date('now')\
            then 'TODAY' else '-' end addl_info, ws.uuid,ws.area
        from workspace ws left join (select uuid ,version,\
            group_concat(tags) as tags from workspace_tags wt\
            group by uuid,version) tg on ws.uuid=tg.uuid and\
            tg.version = ws.version where\
            (ws.uuid, ws.version ) in (values %s)\
            order by modified desc""" % ','.join(["(?,?)"]*int(len(task_uuid_and_version)/2))
    #print(sql_tasks)
    #return
    try:
...
def get_tags(task_uuid, task_version):
    ...
    cur = CONN.cursor()
    sql_tags = """
    select tags from workspace_tags ws where ws.uuid=? and ws.version=?
    """
    try:
        tags = list(cur.execute(sql_tags
...
def modify_task(potential_filters, desc, due, hide, group, tag):
    ...
    for task in task_uuid_and_version:
        ...
        area = result[0][6]
        status = result[0][7]
        results = list(cur.execute("select tags from workspace_tags ws where ws.uuid=? and ws.version=?"\
    ...
    try:
        ...
        for task in task_list:
            if task[3] == TASK_TODO and task[8] == TASK_OVERDUE:
                table.add_row(str(task[0]),str(task[1]),str(task[2]),str(task[3]),str(task[4]),str(task[5]),str(task[6]),str(task[7]),str(task[8]),style=todo_overdue)
            elif task[3] == TASK_TODO and task[8] == TASK_TODAY:
                table.add_row(str(task[0]),str(task[1]),str(task[2]),str(task[3]),str(task[4]),str(task[5]),str(task[6]),str(task[7]),str(task[8]),style=todo_today)
            elif task[3] == TASK_TODO:
                table.add_row(str(task[0]),str(task[1]),str(task[2]),str(task[3]),str(task[4]),str(task[5]),str(task[6]),str(task[7]),str(task[8]),style=todo)            
            elif task[3] == TASK_INPROGRESS and task[8] == TASK_OVERDUE:
                table.add_row(str(task[0]),str(task[1]),str(task[2]),str(task[3]),str(task[4]),str(task[5]),str(task[6]),str(task[7]),str(task[8]),style=inprogress_overdue)
            elif task[3] == TASK_INPROGRESS and task[8] == TASK_TODAY:
                table.add_row(str(task[0]),str(task[1]),str(task[2]),str(task[3]),str(task[4]),str(task[5]),str(task[6]),str(task[7]),str(task[8]),style=inprogress_today)
            elif task[3] == TASK_INPROGRESS:
                table.add_row(str(task[0]),str(task[1]),str(task[2]),str(task[3]),str(task[4]),str(task[5]),str(task[6]),str(task[7]),str(task[8]),style=inprogress)
            elif task[3] == TASK_DONE:
                table.add_row(str(task[0]),str(task[1]),str(task[2]),str(task[3]),str(task[4]),str(task[5]),str(task[6]),str(task[7]),str(task[8]),style=done)            
            else:
                table.add_row(str(task[0]),str(task[1]),str(task[2]),str(task[3]),str(task[4]),str(task[5]),str(task[6]),str(task[7]),str(task[8]),style=todo)
...
def get_and_printactive_task_count(print=True):
    ...
    cur = CONN.cursor()
    sql_cnt = """
        select case when (hide>date('now') and \
        hide is not null) then 'HIDDEN'  else 'VISIBLE' end VSIBILITY,\
        count( distinct uuid) CNT from workspace ws where\
        ws.area='pending' and ws.version = (select max(innrws.version)\
        from workspace \
        innrws where innrws.uuid = ws.uuid) group by case when \
        (hide>date('now') and  hide is not null) then 'HIDDEN'\
        else 'VISIBLE' end  order by VSIBILITY desc\
        """
...
def derive_task_id():
    """
    Get the next available task ID from the active area in the workspace

    Params:
        None

    Returns:
        str:Next available task ID
    """
    global CONN
    cur = CONN.cursor()
...
def get_task_uuid_and_version(potential_filters):
    ...
    if all_tasks:
        """
        When no filter is provided retrieve all tasks from pending 
        area
        """
        ...
        if group is not None:
            #print("for group")
            sql_grp = "select uuid,version from workspace ws where ws.groups like ?\
                and ws.area='pending'"
            params = (group+"%",)
        ...
        if tag is not None:
            #print("for tag")
            tag_list = tag.split(",")
            sql_tag = "select uuid,version from workspace_tags tg where tg.tags \
                in (%s) and tg.uuid in (select innrws.uuid from workspace \
        ...
        sql = "select uuid, version from (%s) unionws where unionws.version =\
...
def get_task_new_version(task_uuid):
    ...
    try:
        results = cur.execute("select max(version) from workspace ws where ws.area = 'pending'\
